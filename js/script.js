...
// Suggerimento dimensioni in base all’immagine
function suggestSizes(imgWidth, imgHeight){
  ...
  commonRatios.forEach(r=>{
    let w=64; let h=Math.round(w/r.ratio);
    sizes.push({w:Math.round(h*r.ratio),h:h});
  });
  const container=document.getElementById("suggestedSizes");
  container.innerHTML="";
  sizes.forEach(s=>{
    const btn=document.createElement("button");
    btn.textContent=`${s.w} x ${s.h}`;
    btn.dataset.width=s.w; btn.dataset.height=s.h;
    btn.classList.add("sizeOption");
    container.appendChild(btn);
  });
  document.querySelectorAll(".sizeOption").forEach(btn=>{
    btn.addEventListener("click", e=>{
      widthStuds=parseInt(e.target.dataset.width);
      heightStuds=parseInt(e.target.dataset.height);
...
upload.addEventListener("change", e=>{
  ...
  img.onload=()=>{
    generatePixels(img.width,img.height);
    ...
    suggestSizes(img.width,img.height);
  }
});

// Pulsante dimensione manuale
...
document.getElementById("generatePDF").addEventListener("click",()=>{
  ...
  doc.setFontSize(12); doc.text(`Dimensione: ${widthStuds} x ${heightStuds} studs`,10,20);

  const canvas=document.createElement("canvas");
  canvas.width=widthStuds; canvas.height=heightStuds;
  const ctx=canvas.getContext("2d");
  pixelData.forEach((p,i)=>{
    const x=i%widthStuds; const y=Math.floor(i/widthStuds);
    ctx.fillStyle=p.hex; ctx.fillRect(x,y,1,1);
    ctx.fillStyle="#000"; ctx.font="1px Arial"; ctx.fillText(p.number,x+0.5,y+0.5);
  });
  ...
  window.LEGO_1x1_PARTS.forEach((p,i)=>{
    const link=`https://www.lego.com/it-it/pick-and-build/pick-a-brick/color/${p.colorName.toLowerCase().replace(" ","-")}`;
    doc.text(`${p.number}: ${p.colorName} | Design ID: ${p.designID} | Element ID: ${p.elementID} | Link: ${link}`,10,yOffset+i*6);
  });
